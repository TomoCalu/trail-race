...
import { getRaceApplications, deleteApplication } from '../services/applicationApi';
  ...
  useEffect(() => {
    ...
    const loadRaceApplications = async () => {
      try {
        ...
        const applied = apps.some(app => app.firstName === user.firstName && app.lastName === user.lastName);
  ...
  const handleDelete = async (appId) => {
    if (window.confirm('Are you sure you want to delete this application?')) {
      try {
        ...
        const applied = updatedApplications.some(app => app.firstName === user.firstName && app.lastName === user.lastName);
        setHasUserApplied(applied);
      } catch (error) {
    ...
    <div className="container mx-auto p-4 max-w-screen-lg">
      ...
      {race && (
        <div className="bg-base-100 shadow-lg rounded-lg mb-8 p-6">
          ...
          <p className="text-lg mb-2"><strong>Distance:</strong> {getDistanceLabel(race.distance)}</p>
          ...
          <Table 
            ...
            data={applications} 
            renderActions={(row) => (
              isApplicant && row.firstName === user.firstName && row.lastName === user.lastName && (
                <button className="btn btn-error btn-sm" onClick={() => handleDelete(row.id)}>
                  Delete
                </button>
              )
            )}
          ...
          {isApplicant && hasUserApplied && (
            <p className="text-red-500 mt-4">You have already applied for this race.</p>
          )}
        </div>
      )}
    </div>
